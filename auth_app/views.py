        ...
        if email_in_cookie:
            ...
            if user and not user.email_verified:
                ...
                try:
                    ...
                    is_verified_in_supabase = check_supabase_verification_status(user.supabase_id, supabase_client)
    ...
    if request.method == 'POST':
        ...
        if hasattr(user, 'email_verified') and not user.email_verified:
            ...
            try:
                ...
                if supabase_client and user.supabase_id:
                    is_verified = check_supabase_verification_status(user.supabase_id, supabase_client)
                    ...
                    # Can't check Supabase - rely on Django status
                    messages.warning(request, 
                        "Your email address has not been verified yet. Please check your inbox for the verification email or "
                        "click the 'Resend Verification Email' button below."
                    )
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        # Create the user in both Django and Supabase
        try:
            ...
            try:
                ...
                # Show appropriate success message with more details
                if email_sent:
                    messages.success(request, 
                    ...
                    messages.warning(request, 
                        ...
                        "Please try to log in and request a new verification email. "
            ...
            # Additional specific error handling
                ...
                # Email service error
                messages.error(request,
                    "We're having trouble with our email service. Your account may have been created, "
                    "but we couldn't send the verification email. Please try to log in, or try again later."
                )
                        else:
...
def check_supabase_verification_status(user_id, supabase_client):
    ...
    try:
        ...
        logger.error(f"Error checking Supabase verification status: {str(e)}")
            return False
